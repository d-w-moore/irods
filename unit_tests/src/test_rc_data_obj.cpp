...
TEST_CASE("#7250: updates to ticket stats do not hang")
{
    ...
    const auto enable_ticket = [](auto& _conn, const auto* _ticket_string) {
        ...
        input.arg2 = const_cast<char*>(_ticket_string);
    ...
    {
        // Create a ticket for reading.
        const auto ticket_string = ticket::client::create_ticket(conn1, ticket::ticket_type::read, data_object.c_str());
    ...
    {
        // Create a ticket for writing.
        const auto ticket_string =
            ticket::client::create_ticket(conn1, ticket::ticket_type::write, data_object.c_str());

        irods::at_scope_exit delete_ticket{
            [&conn1, &ticket_string] { ticket::client::delete_ticket(conn1, ticket_string); }};

        // Set a upper limit on the total number of bytes that can be written to the replica.
        ...
        char new_data[] = "the new data";
        bytesBuf_t write_bbuf{};
        write_bbuf.buf = new_data;
        write_bbuf.len = std::strlen(new_data);
