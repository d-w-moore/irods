...
TEST_CASE("test api resource_info_for_operation")
{
    ...
    auto p = ra::client::resource_info(conn, "demoResc");
    REQUIRE(p.has_value());
    //if (p) {
      auto s =  p->host_name();
      //std::cout << "["<<s<<"]" << "\n";
    ...
    std::string path{"/tempZone/home/rods/thingie"};
    io::odstream out{tp, path, io::leaf_resource_name{"demoResc"}};
}
...
TEST_CASE("dstream", "[iostreams]")
{
    ...
    SECTION("dstream supports intermediate replicas and replica tokens")
    {
        ...
        {
            io::client::default_transport tp1{other_conn}; io::odstream ds1{tp1, path, io::replica_number{ds0.replica_number()}};
            REQUIRE_FALSE(ds1);
        }
